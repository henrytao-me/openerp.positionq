...
class snc_nhan_dinh_tuan(osv.osv):
    ...
    def function_time_format(self, cr, uid, ids, fields, args, context={}):
        ...
        for obj in self.read(cr, uid, ids, ['time']):
            ...
            time = time.strftime(DEFAULT_SERVER_DATE_FORMAT)
            time = self.pool.get('vieterp.utils').get_this_monday(cr, uid, time)            
            res[obj['id']] = time 
        return res
    ...
    def function_calculate(self, cr, uid, ids, fields, args, context={}):
        ...
        for obj in self.read(cr, uid, ids, ['time_format', 'high', 'low', 'trend']):
            ...
            #
            tmp_data = self.pool.get('snc.truc.gia.lme').get_info(cr, uid, from_date, type="tuan")
            if tmp_data:
                vals.update({
                    'real_high': tmp_data.get('high', 0),
                    'real_low': tmp_data.get('low', 0)
                })
                if tmp_data.get('close', 0) > 0:
                    vals.update({
                        'real_trend': '1' if tmp_data.get('close', 0) > tmp_data.get('open', 0) else '-1'                  
                    })    
            vals.update({
    ...
    _name = 'snc.nhan.dinh.tuan'
    _description = 'Nhan Dinh Tuan'
    _columns = {
    ...
    def search(self, cr, uid, args, offset=0, limit=None, order=None, context={}, count=False):
        ...
        res = super(snc_nhan_dinh_tuan, self).search(cr, uid, args, offset=offset, 
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        self.pool.get('snc.nhan.dinh.tuan.setup').check_state_available(cr, uid, context=context)
        #check re-create
        ...
        if vals.get('time'):
            ...
            t = self.pool.get('vieterp.utils').get_this_monday(cr, uid, t)
            now = self.pool.get('vieterp.utils').get_this_monday(cr, uid, now)
            
            if t < now:         
                raise osv.except_osv(_('Cảnh báo!'), _('Không được tạo lại nhận định quá khứ.'))
        #start                    
        res = super(snc_nhan_dinh_tuan, self).create(cr, uid, vals, context=None)
        return res
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        #start
        res = super(snc_nhan_dinh_tuan, self).write(cr, uid, ids, vals, context=context)
        return res
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        #start
        res = super(snc_nhan_dinh_tuan, self).unlink(cr, uid, ids, context=context)
        return res
    ...
    def get_except_date(self, cr, uid, from_date, to_date):
        res = []
        info = self.pool.get('snc.nhan.dinh.tuan.setup').get_info(cr, uid, ['exception'])        
        ids = self.pool.get('snc.nhan.dinh.tuan.exception').search(cr, uid, [('id', 'in', info.get('exception', [])),
                                                                             ('date', '>=', from_date),
                                                                             ('date', '<=', to_date)])
        if ids:
            for obj in self.pool.get('snc.nhan.dinh.tuan.exception').read(cr, uid, ids, ['date']):
                res.append(obj['date'])
        return res
    ...
    def thong_ke(self, cr, uid, from_date, context={}):
        ...
        for user_id in data.copy():                      
            data[user_id].update({
                'diff_abs': round(data[user_id].get('diff_abs'), 2),
                'diff_percent': round(data[user_id].get('diff_percent'), 2),
                'diff_trend': round(data[user_id].get('diff_trend'), 2),
            })
        res = data
        #return
        return res
