...
class snc_nhan_dinh_tuan_setup(osv.osv):
    ...
    def function_dcxh(self, cr, uid, ids, fields, args, context=None):
        ...
        for obj in self.read(cr, uid, ids, ['cal_from', 'cal_to']):
            ...
            tmp_ids = self.pool.get('snc.nhan.dinh.tuan.exception').search(cr, uid, [('id', 'in', info.get('exception', [])),
                                                                                     ('date', '>=', cal_from),
                                                                                     ('date', '<=', cal_to)])
            ...
            res[obj['id']] = int(int(delta.days + 3) / 7) - len(tmp_ids)
        return res
    ...
    _name = 'snc.nhan.dinh.tuan.setup'
    _description = 'Thiet Lap Nhan Dinh Tuan'
    _columns = {
        ...
        'exception': fields.one2many('snc.nhan.dinh.tuan.exception', 'name', string="Các ngày không nhận định"),
        'users': fields.many2many('res.users', 'snc_nhan_dinh_tuan_setup_res_users_rel', 'nid', 'uid', 'Người dùng cần tính trọng số'),
