...
class snc_nhan_dinh_ngay_thong_ke(osv.osv_memory):
    ...
    _description = 'Thong Ke Nhan Dinh Ngay'
    _columns = {
    ...
    def search_range(self, cr, uid, args=None, context=None):
        ...
        #
        from_date = context.get('from_date', now - datetime.timedelta(1))   
    ...
    def thong_ke(self, cr, uid, args=None, context=None):
        ...
        #get users
        users = self.pool.get('snc.nhan.dinh.ngay').get_users_from_range(cr, uid,
        ...
        while i < to_date:
            ...
            thong_ke.append({
                'time': str_i,
                'data': self.pool.get('snc.nhan.dinh.ngay').thong_ke(cr, uid, str_i)
            })
        ...
        if context.get('thong_ke_chi_tiet', True):            
            ...
            while i <= to_date:
                if step > 5:
                    step = 1
                    i += datetime.timedelta(2)
                step += 1
                            
                str_i = datetime.datetime.strftime(i, DEFAULT_SERVER_DATE_FORMAT)
                thong_ke_chi_tiet.append({
                    'time': str_i,
                    'real': self.pool.get('snc.lme').get_info(cr, uid, type="ngay", arg="close", 
                                                              from_date=str_i, init_data=True),
                    'data': self.pool.get('snc.nhan.dinh.ngay').thong_ke_chi_tiet(cr, uid, str_i)
                })
