...
class snc_nhan_dinh_ngay_trong_so(osv.osv_memory):
    
    _name = 'snc.nhan.dinh.ngay.trong.so'
    _description = 'Trong So Nhan Dinh Ngay'
    _columns = {
    ...
    def trong_so(self, cr, uid, args=None, context=None):
        ...
        res = {
            ...
            'ts': self.trong_so_ngay(cr, uid, args, context)
        }
        return res
    ...
    def trong_so_xu_huong(self, cr, uid, args=None, context=None):
        ...
        res = {}
        #get setup info
        info = self.pool.get('snc.nhan.dinh.ngay.setup').get_info(cr, uid, ['cal_from', 
                                                                            'cal_to', 
                                                                            'dcxh'])
        users = self.pool.get('snc.nhan.dinh.ngay.setup').get_users(cr, uid)
        if not info:
            return res
        thong_ke = self.pool.get('snc.nhan.dinh.ngay.thong.ke').thong_ke(cr, uid, context={
    ...
    def trong_so_ngay(self, cr, uid, args=None, context=None):
        if not context:
            context = {}
        res = {}
        info = self.pool.get('snc.nhan.dinh.ngay.setup').get_info(cr, uid, ['percent_xh', 'percent_bd'])
        tsxh = self.trong_so_xu_huong(cr, uid, args, context)
        tsbd = self.trong_so_bien_do(cr, uid, args, context)
        res = {}
        for u in tsxh['data']:
